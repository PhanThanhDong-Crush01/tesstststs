...
export const phanPhatVouher = async (top10User: any) => {
    try {
        ...
        alert(error?.response?.data?.message + '!')
        console.log(`['phanPhatVouher']`, error)
    }
}
    ...
    <div style="max-width: 600px; margin: 0 auto;" class="email-container">
        <div valign="middle" class="hero2 middleChi">
            <div class="text middleChiText" style="text-align: center;">
                <h3>Chúc mừng bạn đã nhận được voucher mới của chúng tôi</h3>
                <h4>Hãy đi đến cửa hàng của chúng tôi để sử dụng ngay voucher độc đắc! Bạn có thể click vào nút bên dưới
                    để bắt đầu mua sắm</h4>
                <p><a href="http://localhost:5173/my_voucher" class="btn btn-primary andi">Đi đến xem voucher của
                        tôi</a></p>
            </div>
        </div>
        <table align="center" role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%"
            ...
            <tr>
                <td valign="middle" class="hero bg_white" style="padding: 3em 0 2em 0;">
                    <img src="https://res.cloudinary.com/dsi8kycrz/image/upload/v1712271720/In_Phi%E1%BA%BFu_qu%C3%A0_t%E1%BA%B7ng_voucher_gi%C3%BAp_qu%E1%BA%A3ng_b%C3%A1_th%C6%B0%C6%A1ng_hi%E1%BB%87u_%C4%91%E1%BA%BFn_kh%C3%A1ch_h%C3%A0ng_1_a48w6q.png"
                        alt=""
...
export const add = async (voucher: IVoucher) => {
    try {
        ...
        if (response.data) {
            ...
            const auth = data?.data?.users
            auth.map(async (user: any) => {
                sendd(user?.email)
            })
        }
        return response.data
